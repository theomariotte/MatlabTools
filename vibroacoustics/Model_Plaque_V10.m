...
% - Tentative de mise en place du calcul de l'efficacité modale
% - Résoudre le pb avec les nombre d'onde -->  C.Pezerat ?

% Sur interface graphique : Mettre les légende 
%                           Ajouter le tableau des modes plaque en
%                           fréquence naturelle
%                           Corriger bugs qui allonge le temps de calcul
%--------------------------------------------------------------------------
%%                            Nettoyage
%--------------------------------------------------------------------------
close all;
...
f = 1700;     
f_p=500:5:2000; % Vecteur fréquence
...
for ii=1:length(phi_chapeau3)
    for jj=1:length(phi_chapeau3)
        moy_phi3(ii,jj)=mean(abs(phi_chapeau3{ii,jj}),'All');
    end
end
%%                               Affichage 
%--------------------------------------------------------------------------
...
%% Fenetre 2
% h2 = figure('Name','');
% plot(kx,phi_chapeau2(1,:));
% grid on;

%% Fenetre 3
% h3 = figure('Name','');
% surf(abs(phi_chapeau2))
% shading('interp')
